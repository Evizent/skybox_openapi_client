...
class VenueTimeZoneRules(BaseModel):
    """
    VenueTimeZoneRules
    """ # noqa: E501
    fixed_offset: Optional[StrictBool] = Field(default=None, alias="fixedOffset")
    transitions: Optional[List[VenueTimeZoneRulesTransitionsInner]] = None
    transition_rules: Optional[List[VenueTimeZoneRulesTransitionRulesInner]] = Field(default=None, alias="transitionRules")
    __properties: ClassVar[List[str]] = ["fixedOffset", "transitions", "transitionRules"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in transition_rules (list)
        _items = []
        if self.transition_rules:
            for _item_transition_rules in self.transition_rules:
                if _item_transition_rules:
                    _items.append(_item_transition_rules.to_dict())
            _dict['transitionRules'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "fixedOffset": obj.get("fixedOffset"),
            "transitions": [VenueTimeZoneRulesTransitionsInner.from_dict(_item) for _item in obj["transitions"]] if obj.get("transitions") is not None else None,
            "transitionRules": [VenueTimeZoneRulesTransitionRulesInner.from_dict(_item) for _item in obj["transitionRules"]] if obj.get("transitionRules") is not None else None
        })
        return _obj
