...
class MergeInventoryRequest(BaseModel):
    ...
    """ # noqa: E501
    inventory_ids: Annotated[List[StrictInt], Field(min_length=2, max_length=2147483647)] = Field(description="Inventory id's which will be merged.", alias="inventoryIds")
    public_notes: Optional[StrictStr] = Field(default=None, description="Public notes of the merged inventory.", alias="publicNotes")
    merging_type: Optional[StrictStr] = Field(default=None, description="Strategy to merge public notes in the inventory", alias="mergingType")
    __properties: ClassVar[List[str]] = ["inventoryIds", "publicNotes", "mergingType"]

    @field_validator('merging_type')
    def merging_type_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['COMBINE', 'WIPE', 'NEW']):
            raise ValueError("must be one of enum values ('COMBINE', 'WIPE', 'NEW')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "mergingType": obj.get("mergingType")
        })
        return _obj
