...
class TagColorUpdateRequest(BaseModel):
    ...
    color_value: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, alias="colorValue")
    __properties: ClassVar[List[str]] = ["tagName", "colorValue"]

    @field_validator('color_value')
    def color_value_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^#([a-zA-Z0-9]){3}(([a-zA-Z0-9]){3})?$", value):
            raise ValueError(r"must validate the regular expression /^#([a-zA-Z0-9]){3}(([a-zA-Z0-9]){3})?$/")
        return value
