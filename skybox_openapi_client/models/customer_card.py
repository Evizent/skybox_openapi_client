...
class CustomerCard(BaseModel):
    ...
    last_four_digits: StrictInt = Field(alias="lastFourDigits")
    external_id: StrictStr = Field(alias="externalId")
    customer_external_id: StrictStr = Field(alias="customerExternalId")
    address_external_id: StrictStr = Field(alias="addressExternalId")
    cardholder_name: Optional[StrictStr] = Field(default=None, alias="cardholderName")
    expiration_month: Optional[StrictInt] = Field(default=None, alias="expirationMonth")
    expiration_year: Optional[StrictInt] = Field(default=None, alias="expirationYear")
    customer_id: Optional[StrictInt] = Field(default=None, description="Required for updates, ignored on inserts.", alias="customerId")
    default_card: Optional[StrictBool] = Field(default=None, alias="defaultCard")
    last_date_used: Optional[datetime] = Field(default=None, alias="lastDateUsed")
    __properties: ClassVar[List[str]] = ["id", "accountId", "cardType", "lastFourDigits", "externalId", "customerExternalId", "addressExternalId", "cardholderName", "expirationMonth", "expirationYear", "customerId", "defaultCard", "lastDateUsed"]

    @field_validator('card_type')
    def card_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['AMEX', 'DISCOVER', 'MASTERCARD', 'UNKNOWN', 'VISA']):
            raise ValueError("must be one of enum values ('AMEX', 'DISCOVER', 'MASTERCARD', 'UNKNOWN', 'VISA')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "defaultCard": obj.get("defaultCard"),
            "lastDateUsed": obj.get("lastDateUsed")
        })
        return _obj
