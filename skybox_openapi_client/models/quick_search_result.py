...
class QuickSearchResult(BaseModel):
    ...
    matched_field: Optional[StrictStr] = Field(default=None, alias="matchedField")
    external_ref: Optional[StrictStr] = Field(default=None, alias="externalRef")
    __properties: ClassVar[List[str]] = ["entityId", "entityType", "matchedField", "externalRef"]
    ...
    def entity_type_validate_enum(cls, value):
        ...
        if value not in set(['INVOICE', 'PURCHASE', 'INVENTORY', 'HOLD']):
            raise ValueError("must be one of enum values ('INVOICE', 'PURCHASE', 'INVENTORY', 'HOLD')")
        return value
